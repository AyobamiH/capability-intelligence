...
| `CI-011` | Decide ownership, licence, and distribution | John selected MIT, `AyobamiH/capability-intelligence`, and npm identity `capability-intelligence@0.1.0` | `relevant-ready` | P0 | Verify public remote, exact-commit CI, and npm registry result | MIT file and public metadata exist; package contents pass review; remote and registry identities are verified independently. |
...
`CI-011` is the active bounded publication workpack. After public remote, CI, and registry verification, the next evidence-backed investigation is `CI-008`, but it should not be implemented until more than one real receipt producer exists.
